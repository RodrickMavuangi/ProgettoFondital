...
namespace Fondital.Server.Controllers
{
    ...
    public class MailController : ControllerBase
    {
        ...
        [HttpPost("addUser")]
        public async Task<IActionResult> AddUtente([FromBody] UtenteDto utenteDto)
        {
            Utente utente = _mapper.Map<Utente>(utenteDto);
            try
            {
                //utente = await _utService.CreateUtente(utente);
                await _userManager.CreateAsync(utente);
                utente = await _userManager.FindByNameAsync(utente.UserName);
